...
export class StackItem extends ContentItem {
  ...
  _createTabElement (child, index) {
    const tab = document.createElement('div')
    tab.className = 'tulweb-tab'
    ...
    if (isCloseable) {
      ...
      close.addEventListener('click', (e) => {
        e.stopPropagation()
        // Get current index at click time
        const currentIndex = this.children.indexOf(child)
        this._closeTab(child, currentIndex)
      })
    }
    ...
    tab.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
        e.preventDefault()
        this.setActive((currentIndex + 1) % this.children.length)
        this.tabsEl.querySelectorAll('.tulweb-tab')[this.activeChildIndex].focus()
      } else if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
        e.preventDefault()
        this.setActive((currentIndex - 1 + this.children.length) % this.children.length)
        this.tabsEl.querySelectorAll('.tulweb-tab')[this.activeChildIndex].focus()
      } else if (e.key === 'Home') {
        e.preventDefault()
        this.setActive(0)
        ...
        e.preventDefault()
        this._closeTab(child, currentIndex, true)
      }
    ...
    tab.addEventListener('contextmenu', (e) => {
      ...
      const currentIndex = this.children.indexOf(child)
      this.setActive(currentIndex)
      this.layoutManager.showContextMenu(e, this, child, currentIndex)
    })
  ...
  renderTabs () {
    ...
    this.children.forEach((child, index) => {
      ...
      // Sync active state
      const isActive = index === this.activeChildIndex
      child._tabEl.classList.toggle('active', isActive)
      child._tabEl.setAttribute('aria-selected', isActive ? 'true' : 'false')
      child._tabEl.setAttribute('tabindex', isActive ? '0' : '-1')

      fragment.appendChild(child._tabEl)
      if (currentTabElements[index] !== child._tabEl) {
        changed = true
      }
    })

    if (changed || currentTabElements.length !== this.children.length) {
      this.tabsEl.textContent = ''
      this.tabsEl.appendChild(fragment)
    } else {
      // Elements are back in the fragment, must put them back if we moved them
      // But actually, fragment.appendChild ALREADY moved them out of tabsEl.
      this.tabsEl.appendChild(fragment)
    }
  }
