...
export class Splitter {
  constructor (isVertical, prevItem, nextItem, parentContainer) {
    ...
    this.element.setAttribute('aria-label', `Resize ${isVertical ? 'columns' : 'rows'}`)
  ...
  _moveSplitter (diffPx) {
    ...
    if (prevType === 'weight' && nextType === 'weight') {
      ...
      const newPrev = startPrevSize + diffPct
      const newNext = startNextSize - diffPct
  ...
  _updateAriaValues () {
    const size = typeof this.prevItem.size === 'number' ? this.prevItem.size : Utils.parseSize(this.prevItem.size).value
    this.element.setAttribute('aria-valuenow', Math.round(size))
  ...
  _onMouseDown (e) {
    if (this.element.classList.contains('adjacent-minimized')) return
